...
<article class="blog-item">
	...
	{*<div class="blog-meta"></div>*}
		{if $config->get('listing_show_title','1')}
			<h4 class="title"><a href="{$blog.link|escape:'html':'UTF-8'}" title="{$blog.title|escape:'html':'UTF-8'}">{$blog.title|escape:'html':'UTF-8'|truncate:50:'...' nofilter}</a></h4>
		{/if}
		
		<div class="blog-shortinfo">
			{if $config->get('listing_show_description','1')}
			{$blog.description|strip_tags:'UTF-8'|truncate: 85:'...' nofilter}{* HTML form , no escape necessary *}
			{/if}
			...
			<span class="blog-ctncomment">
				...
				{$blog.comment_count|intval} {l s='Comment' mod='psblog'}
			</span>
			{/if}		
