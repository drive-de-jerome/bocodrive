...
{if isset($p) AND $p}
	...
	{if $start!=$stop}
		...
		{else}
			<li id="pagination_previous{if isset($paginationId)}_{$paginationId|escape:'html':'UTF-8'}{/if}" class="previous disabled pagination_previous"><span class="previous"><i class="fa fa-long-arrow-left"></i> </span></li>
		{/if} 
		{if $start==3}
		...
		{if $pages_nb > 1 AND $p != $pages_nb}
			...
			<li id="pagination_next{if isset($paginationId)}_{$paginationId|escape:'html':'UTF-8'}{/if}" class="pagination_next">	
				<a {$no_follow_text|escape:'html':'UTF-8'} href="{$link->goPage($requestPage, $p_next)|escape:'html':'UTF-8'}" class="next"><i class="fa fa-long-arrow-right"></i></a>
			</li>
		{else}
			<li id="pagination_next{if isset($paginationId)}_{$paginationId|escape:'html':'UTF-8'}{/if}" class="next disabled pagination_next"><span class="next"> <i class="fa fa-long-arrow-right"></i></span></li>
		{/if}
		</ul>
	{/if}
	</div>
    ...
    <div class="product-count float-xs-right">
		...
		{if $nb_items > 1}
			{l s='Showing' mod='psblog'} {$blogShowingStart} - {$blogShowing} {l s='of' mod='psblog'} {$nb_items} {l s='items' mod='psblog'}	
		{else}
			{l s='Showing' mod='psblog'} {$blogShowingStart} - {$blogShowing} {l s='of' mod='psblog'} 1 {l s='item' mod='psblog'}
		{/if}
