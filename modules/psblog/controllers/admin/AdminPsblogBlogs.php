...
class AdminPsblogBlogsController extends ModuleAdminController
{
    ...
    public function __construct()
    {
        ...
        $this->bulk_actions = array('delete' => array('text' => $this->trans('Delete selected'), 'confirm' => $this->trans('Delete selected items?'), 'icon' => 'icon-trash'));
        $this->fields_list = array(
            'id_psblog_blog' => array('title' => $this->trans('ID'), 'align' => 'center', 'class' => 'fixed-width-xs'),
            'meta_title' => array('title' => $this->trans('Blog Title'), 'filter_key' => 'b!meta_title'),
            'title' => array('title' => $this->trans('Category Title'), 'filter_key' => 'cl!title'),
            'active' => array('title' => $this->trans('Displayed'), 'align' => 'center', 'active' => 'status', 'class' => 'fixed-width-sm', 'type' => 'bool', 'orderby' => false)
        );
    ...
    public function initPageHeaderToolbar()
    {
        ...
        if (Tools::getValue('id_psblog_blog')) {
            $this->page_header_toolbar_btn['save-and-preview'] = array(
                ...
                'desc' => $this->trans('Manage Comments'),
...
//      $tree = $obj->getTree();    # validate module
        ...
        $this->fields_form[0]['form'] = array(
            ...
            'input' => array(
                ...
                array(
                    'type' => 'textarea',
                    'label' => $this->trans('Video Code'),
                    'name' => 'video_code',
                    'rows' => 5,
                    'cols' => 30,
                    'hint' => $this->trans('Enter Video Code Copying From Youtube, Vimeo').' <>;=#{}'
                ),
                array(
                    'type' => 'textarea',
                    'label' => $this->trans('Blog description'),
                    'name' => 'description',
                    'autoload_rte' => true,
                    'lang' => true,
                    'rows' => 5,
                ...
                array(
                    'type' => 'switch',
                    'label' => $this->trans('Indexation (by search engines):'),
                    'name' => 'indexation',
                    'required' => false,
                    'class' => 't',
                    'is_bool' => true,
    ...
    {
        $this->toolbar_title = $this->trans('Blogs Management');
    ...
    public function displayList($token = null)
    {
        ...
        if (!count($this->_list)) {
            echo '<tr><td class="center" colspan="'.(count($this->fields_list) + 2).'">'.$this->trans('No items found').'</td></tr>';
        }
