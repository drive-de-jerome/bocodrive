...
class AdminPsblogCategoriesController extends ModuleAdminController
{
    ...
    public function __construct()
    {
		...
		$this->toolbar_title = $this->trans('Categories Management');
		// echo '<pre>';
		// print_r($this->trans('Ps Blog Management'));die();
    }
    ...
    public function postProcess()
    {
        if (Tools::getValue('doupdatepos') && Tools::isSubmit('updatePosition')) {
            ...
            die($this->trans('Update Positions Done'));
        }
	    ...
	    $id_shop = (int)Context::getContext()->shop->id;
            ...
            if ($megamenu->validateFields(false) && $megamenu->validateFieldsLang(false)) {
                ...
                // validate module
                $this->_html .= '<div class="conf error alert alert-warning">'.$this->trans('An error occurred while attempting to save.').'</div>';
            }
        }
    }
    ...
    public function renderList()
    {
        ...
        $this->fields_form[0]['form'] = array(
            'legend' => array(
                'title' => $this->trans('Category Form.'),
            ),
            'input' => array(
                array(
                    'type' => 'hidden',
                    'label' => $this->trans('Category ID'),
                    'name' => 'id_psblogcat',
                    'default' => 0,
                ),
                array(
                ...
                array(
                    ...
                    'lang' => true,
                    'default' => '',
                    'hint' => $this->trans('Only letters and the minus (-) character are allowed')
                ),
                array(
                    'type' => 'select',
                    'label' => $this->trans('Parent ID'),
                    'name' => 'id_parent',
                ...
                array(
                    'type' => 'text',
                    'label' => $this->trans('Addion Css Class'),
                    'name' => 'menu_class',
                    'display_image' => true,
                    'default' => ''
                ),
                array(
                    'type' => 'text',
                    'label' => $this->trans('Menu Icon Class'),
                    'name' => 'icon_class',
                    'display_image' => true,
                    'default' => '',
                    'desc' => $this->trans('The module integrated with FontAwesome').'. '
                    .$this->trans('Check list of icons and class name in here')
        ...
        $helper->title = $this->trans('Categories Management');
        ...
        $output = $html.'
	        ...
	        ';
        $output .= '<div class="col-md-4"><div class="panel panel-default"><h3 class="panel-title">'.$this->trans('Tree Blog Categories Management').'</h3>'
                .'<div class="panel-content">'.$this->trans('To sort orders or update parent-child, you drap and drop expected menu.')
                .'<hr><p><input type="button" value="'.$this->trans('New Category').'" id="addcategory" data-loading-text="'.$this->trans('Processing ...').'" class="btn btn-danger" name="addcategory"></p><hr>'.$tree.'</div></div></div>'
