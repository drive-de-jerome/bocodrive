...
class stfeature extends Module
{
    ...
    public function __construct()
    {
        ...
        $this->displayName = $this->l('CZ Feature');
        $this->description = $this->l('CZ feature for prestashop 1.7: compare, wishlist at product list');
    ...
    public function install()
    {
		...
		if (parent::install() && $this->registerStHook())
        {
            ...
            foreach ($langs as $l) {
                # validate module
                $tab1->name[$l['id_lang']] = $this->l('Codezeel Wishlist & Compare Management');
            }
//            $id_tab1 = $tab1->add(true, false);
			...
			$this->installModuleTab('Wishlist & Compare Configuration', 'module', 'AdminStfeatureManagement');
    ...
    */
	...
	//DONGND:: add mywishlist link to top Header section
	public function hookdisplayStWishlistTopLink($params)
    {
		if (Configuration::get('STFEATURE_ENABLE_PRODUCTWISHLIST'))
		{
			$this->context->smarty->assign(array(
			...
			return $this->display(__FILE__, 'st_wishlist_toplink.tpl');
		}
	}

	public function hookdisplayStCompareTopLink($params)
    {
		if (Configuration::get('STFEATURE_ENABLE_PRODUCTCOMPARE'))
		{
			$this->context->smarty->assign(array(
				'compare_link' => $this->link->getModuleLink('stfeature', 'productscompare'),            
			));
			return $this->display(__FILE__, 'st_compare_toplink.tpl');
		}
	}
    ...
    public function registerStHook()
    {
		...
		$res &= $this->registerHook('displayStWishlistTopLink');
		$res &= $this->registerHook('displayStCompareTopLink');
    ...
    public function unregisterStHook()
    {
		...
		$res &= $this->unregisterHook('displayStWishlistTopLink');
		$res &= $this->unregisterHook('displayStCompareTopLink');
